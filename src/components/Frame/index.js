      ...
      if (breadcrumbNameMap[url]) {
        return (
          <Breadcrumb.Item key={url}>
            <Link
              to={breadcrumbNameMap[url].route ? "#" : url}
              style={{ fontWeight: "bold" }}
            >
              <IconFont
                type={breadcrumbNameMap[url].icon}
      ...
      if (item.isShow) {
        ...
        return (
          <Menu.Item
            ...
            key={item.path}
            icon={<IconFont type={item.icon} />}
            onClick={(p) => this.toRouteView(item)}
          >
  ...
  newTabs = (routers) => {
    for (let i in routers) {
      if (
        !routers[i].childrens &&
        routers[i].isShow &&
        routers[i].path === this.state.pathname
      ) {
        // 判断当前路由是否已经存在标签页数组中
      ...
      <Layout>
        ...
        <Layout className="site-layout">
          ...
          >
            ...
            {/* 面包屑 */}
            <Breadcrumb
              style={{ margin: "21px 0", padding: "0 24px", float: "left" }}
            >
              ...
              <SubMenu
                ...
                title={
                  <Fragment>
                    ...
                    <span
                      style={{ color: "#999", marginRight: 4, marginLeft: 5 }}
                    >
                      <Trans>你好</Trans>,
                    </span>
              ...
              {this.state.panes.map((pane) => (
                <TabPane
                  tab={
                    <span>
                      {" "}
                      <IconFont type={pane.icon} /> <Trans>{pane.title}</Trans>{" "}
                    </span>
                  }
                  key={pane.path}
